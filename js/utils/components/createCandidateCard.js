...
export function createCandidateCard(candidate) {
    ...
    Promise.all(categoryPromises).then((categoryNames) => {
        ...
        const voteAnchor = document.createElement("a");
        voteAnchor.className = "bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition duration-300 mr-2";
        voteAnchor.textContent = "Vote";
        voteAnchor.href = `./pages/vote-candidate.html?candidate=${id}&event=${event_id}&categories=${category_ids.join(",")}`;
        voteAnchor.setAttribute("role", "button");
        ...
        voteContainer.appendChild(voteAnchor);
...
export function createCandidateCard2(candidate) {
    ...
    Promise.all(categoryPromises).then((categoryNames) => {
        ...
        voteAnchor.textContent = "Vote";
        voteAnchor.href =
            `./vote-candidate.html?candidate=${id}&event=${event_id}&categories=${category_ids.join(",")}`;
        voteAnchor.setAttribute("role", "button");
        const votes = document.createElement("span");
        votes.className = "text-gray-600";
        votes.textContent = "Votes: 0";
